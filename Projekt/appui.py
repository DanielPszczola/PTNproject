...
class UserInterface:
    ...
    def create_widgets(self):
        ...
        self.plot_label = tk.Label(self.root, text="Plot Type:")
        self.plot_label.pack()
        self.plot_type = ttk.Combobox(self.root, values=['count', 'scatter', 'histogram', 'box', 'bar', 'heatmap'])
        self.plot_type.pack()
            ...
            if plot_type:
                try:
                    if plot_type == 'count':
                        ...
                        if column:
                            GraphGen.plot_count(self.data, column)
                        else:
                        ...
                        if x_column and y_column:
                            GraphGen.plot_scatter(self.data, x_column, y_column)
                        else:
                            messagebox.showerror("Error", "Both X and Y columns must be selected for scatter plot.")
                    elif plot_type == 'histogram':
                        column = self.plot_column.get()
                        if column:
                            GraphGen.plot_histogram(self.data, column)
                        else:
                            messagebox.showerror("Error", "Plot column must be selected for histogram.")
                    elif plot_type == 'box':
                        column = self.plot_column.get()
                        if column:
                            GraphGen.plot_box(self.data, column)
                        else:
                            messagebox.showerror("Error", "Plot column must be selected for box plot.")
                    elif plot_type == 'bar':
                        column = self.plot_column.get()
                        if column:
                            GraphGen.plot_bar(self.data, column)
                        else:
                            messagebox.showerror("Error", "Plot column must be selected for bar plot.")
                    elif plot_type == 'heatmap':
                        GraphGen.plot_heatmap(self.data)
                    else:
